...
save: coe_save/coe_constants.s coe_save/coe_macros.s coe_save/coe_rop.s coe_ropdb/$(REGION)_$(COE_VERSION)_ropdb.txt
	...
	@python scripts/makeHeaders.py constants/constants "FIRM_VERSION=$(FIRM_VERSION)" "COE_SLOT=$(COE_SLOT)" "COE_VERSION=$(COE_VERSION)" coe_ropdb/$(REGION)_$(COE_VERSION)_ropdb.txt
	@cd coe_code && $(MAKE)
