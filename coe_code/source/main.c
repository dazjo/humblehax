...
Result payload_update(u8* firmware_version)
{
    ...
    char user_agent[] = "humblehax_menu";
    ...
    if(ret || (padGet() & BUTTON_SELECT))
    {
        ...
        while(1)
        {
            switch(state)
            {
                case 0:
                    select = 0;
                    centerString(top_framebuffer, "humblehax menu - SALT", 24, 400);
...
updateFail:
    ...
    // run payload
    {
        void (*payload)(u32* paramlk, u32* stack_pointer) = (void*)PAYLOAD_VA;
        u32* paramblk = (u32*)LINEAR_BUFFER;

        paramblk[0x1c >> 2] = COE_GSPGPU_GXCMD4;
        paramblk[0x20 >> 2] = COE_GSPGPU_FLUSHDATACACHE_WRAPPER;
        paramblk[0x48 >> 2] = 0x8d; // flags
        paramblk[0x58 >> 2] = COE_GSPGPU_HANDLE;
